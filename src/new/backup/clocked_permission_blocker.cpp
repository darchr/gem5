...
//                     // must be of 64 bytes.
//                     int memory_packes_required = 23; // (permissionEntrySize / 64);
...
//                             DPRINTF(PermissionPackets, "Send %#x for %#x on port %lu count %d\n",
//                                                     permission_pkt->getAddr(), pkt->getAddr(), packet_id, i);

//                         }
//                         // TODO: can this packet go into the same retry queue?
//                         else {
//                             DPRINTF(PermissionPackets, "Couldn't send %#x for %#x on port %lu count %d\n",
//                                                     permission_pkt->getAddr(), pkt->getAddr(), packet_id, i);
//                             // only store the actual request and delete the fake request.
//                             // it'll be created again.
//                             //
...
ClockedPermission::recvTimingReq(PacketPtr pkt, uint64_t packet_id) {
    ...
    if (enablePermissionCheck) {
        ...
        if (useDedicatedCaching) {
            ...
            // permission checks only happen for remote memory addresses!
            // TODO: Fix hardcoding issues
            if (is_remote && permission_checker[pkt->getAddr()] <= 23) {
                // this request can be either in the data section or the table
                if (pkt->getAddr() >= baseAddrPermissionTable &&
                pkt->getAddr() < baseAddrPermissionTable + 0x40000000) {
                    // This is an OS request to read or write into the
                    ...
                    for (int i = 0 ; i < memory_packes_required ; i++) {
                        ...
                        // First create a new request
                        // FIXME: There needs to be n number of read requests by 64.
                        Addr permission_addr = baseAddrPermissionTable;//; getBinarySearchPermissionTableAddr(permission_checker[pkt->getAddr()]);
                        // Request::Flags flags;
                        RequestPtr req = std::make_shared<Request>(
                                            permission_addr,
                                            1,
    ...
    if (is_remote) {
        // wait until all the permission packets are sent first.
        if (permission_checker[pkt->getAddr()] < 23) {
            // a permission packet was sent and so was the actual packet
            if (did_permission_failed == 0x2)
                DPRINTF(PermissionPackets, "Permission packet number %d for %#x "
                                            "was sent but the actual packet "
                                            "is explicitly stalled!\n",
                                            permission_checker[pkt->getAddr()],
                                            pkt->getAddr());
            else if (did_permission_failed == 0x1)
                DPRINTF(PermissionPackets, "Permission packet number %d for %#x "
                                            "failed and the actual packet "
                                            "is explicitly stalled!\n",
                                            permission_checker[pkt->getAddr()],
                                            pkt->getAddr());
            else {
            ...
            // this pacet cannot go through as i just enqueued a permission
            // packet previously
            assert(memSidePort.sendTimingReq(pkt) == false);

            // ive seen this packet for the first time.
            // if (retry_tracker[pkt->getAddr()] == false) {
                // retry_tracker[pkt->getAddr()] = true;
            // }
            ...
            // the main packet is ready to be sent.
            assert(did_permission_failed == 0x3);
            if (memSidePort.sendTimingReq(pkt)) {
                // business as usual!
                ...
                // this packet must have been tracked for retry
                // if (retry_tracker[pkt->getAddr()] == true)
                //     retry_tracker[pkt->getAddr()] = false;
                // else {
                //     assert(false && "logical error: this packet must have been"
                //                     " tracked for retry\n");
                // }
                return true;
            }
            // this time it really failed!
            DPRINTF(PermissionPackets, "Real packet for %#x on port %lu!"
                                       " failed to be sent!\n", pkt->getAddr(),
                                                                packet_id);
            retry_queue.push(packet_id);
            return false;
        ...
        // this is a regular request. business as usual.
        if (memSidePort.sendTimingReq(pkt)) {
...
//         // the rest of the shit only deals with remote memory packets.
//         if (is_remote) {
//             if (permission_checker[pkt->getAddr()] >= 23) {
//                 // if (did_permission_failed == 0x2) {
//                 // a permission packet was sent and so was the actual packet
//                 DPRINTF(PermissionPackets, "Both permission and actual packet "
//                                             "sent for %#x\n", pkt->getAddr());
//                 // if (permission_checker[pkt->getAddr()] == 24)
//                 // reset the count so that the request needs to fetch the 
//                 // permission entry again.
//                 permission_checker[pkt->getAddr()] = 0;
//                 // this packet must have been tracked for retry
...
//                 }
//             }
//             else if (permission_checker[pkt->getAddr()] < 23) {
//                 // the real packets should not be sent!
//                 assert(false && "until all the permission packets are sent,"
//                                 " the real packet should not be sent!\n");
//             }
//         }
//         return true;
//     }

//     // the real packet failed! you have to retry!
//     if (is_remote) {
//         // make sure that the retyr is done only once.
//         if (permission_checker[pkt->getAddr()] > 0 &&
//                                     permission_checker[pkt->getAddr()] <= 23) {
//             // a permission packet was sent and so was the actual packet
//             DPRINTF(PermissionPackets, "Permission packet number %d for %#x "
//                                         "was sent but the actual packet "
//                                         "failed!\n",
...
//         else {
//             // a permission packet was sent and so was the actual packet
//             DPRINTF(PermissionPackets, "All permission packets (%d) are sent "
//                                         "but the real packet for %#x is still "
//                                         "failed!\n",
//                                             permission_checker[pkt->getAddr()],
//                                             pkt->getAddr());
//         }
...
//     // regardless of what happened, this function will still return false as
//     // the actual packet was failed to be sent!.
//     return false;
// }
